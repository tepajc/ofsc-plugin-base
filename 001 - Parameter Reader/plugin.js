...
(function ($) {
    ...
    $.extend(window.OfscPlugin.prototype, {
        ...
        pluginOpen: function (receivedData) {
          ...
          var content = "<pre> Looking for activities for "+ info.resource.pname+ " ("+ info.resource.external_id+ ") "+ received + "</pre>";
          ...
          /// find parent bucket for calling resources
          var request = baseURL + "/resources/"+ info.resource.external_id;
          $.getJSON(request, {"fields": "resourceType, parentResourceId"}).done(function(data){
              console.log(JSON.stringify(data));
          });
          console.log("Despues de pedir el recurso");
          ...
          function distance_between(lat1, lon1, lat2, lon2) {
            ...
            var φ1 = lat1*0.01745329251;
            var φ2 = lat2*0.01745329251;
            var Δφ = (lat2 - lat1)*0.01745329251;
            var Δλ = (lon2 - lon1)*0.01745329251;
          ...
          function drawMap() {
            ...
            mapData.addColumn("string", "resourceId");
            mapData.addColumn("string", "activityDate");
            mapData.addColumn("string", "Distance");
            mapData.addColumn("number", "Position In Route");
            mapData.addColumn("string", "apptNumber");
            mapData.addColumn("string", "startTime");
            request = baseURL + "/resources/"+ info.resource.external_id + "/findNearbyActivities";
            $.getJSON( request, {"fields": "activityId, resourceId, date, status, latitude, longitude, positionInRoute, customerName, apptNumber, startTime"}, function( data ) {
                console.log( "success 3 in getJSON" );
                        body.innerHTML +="<pre> FOUND: "+JSON.stringify(data, null, 4)+"</pre>";
                data.items.forEach((item, i) => {
                      var distance = distance_between(data.coordinates.latitude, data.coordinates.longitude, item.latitude, item.longitude);
                      mapData.addRows([
                        [item.latitude, item.longitude, item.activityId.toString(), "green", item.resourceId, item.date, distance.toString(), item.positionInRoute, item.apptNumber.toString(), item.startTime]
                      ]);
                });
                mapData.addRows([
                  [data.coordinates.latitude, data.coordinates.longitude, "N/A", "tech", info.resource.external_id, "", "", 0, "", ""]
                ]);
                ...
                var options = {
                  ...
                  tech: {
                    normal : 'http://icons.iconarchive.com/icons/icons8/windows-8/48/Ecommerce-Delivery-icon.png',
                    selected: 'http://icons.iconarchive.com/icons/icons8/windows-8/48/Ecommerce-Delivery-icon.png'
                  }
                }
              };
                ...
                google.visualization.events.addListener(table, 'select', function() {
                  ...
                  alert('You selected ' + mapData.getValue(row, 2));
                });


                // Create a view that shows everyone hired since 2007.
                var view = new google.visualization.DataView(mapData);
                view.setColumns([0,1,2,3]);
                var map = new google.visualization.Map(document.getElementById('map_div'));
                map.draw(view, options);

                // Create a view that shows everyone hired since 2007.
                var tableview = new google.visualization.DataView(mapData);
                tableview.setColumns([8,4,9,6,7]);
                table.draw(tableview, {sortColumn: 1});
            });
            };
        },
