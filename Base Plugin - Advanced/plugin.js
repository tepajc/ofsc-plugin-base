...
(function ($) {
    ...
    $.extend(window.OfscPlugin.prototype, {
        ...
        pluginOpen: function (receivedData) {
            ...
            this.initChangeOfOpenOption('back-navigation', 'backNavigationFlag');
            this.initChangeOfOpenOption('show-header', 'showHeader');
 
            $('.button--generate-sign').on('click', function (e) {
            ...
            $('.back_method_select, .back_activity_id, .back_inventory_id, .back_plugin_label, .back_plugin_link_id, .back_plugin_params').on('change', function () { //IE10+
                this._updateResponseJSON();
            }.bind(this));
            ...
            $('.submit').click(function () {
                ...
                if (json_response.is(":hidden") === true) {
                    ...
                    if (this._isJson(json_response.text())) {
                        data = JSON.parse(json_response.text());
                    } else {
                ...
                this._saveWakeupData();
 
                this._sendPostMessageData(data);
            }.bind(this));
 
            this.initAddButtons(document);
            this.initProcedureActions(document);
 
            this._updateResponseJSON(); //create json for first time that user can view it
 
            $('.section__ofsc-data').show();
 
            $('h2').click(function () {
                $(this).toggleClass('collapsed');
                $(this).closest('.section').toggleClass('section--collapsed');
            });
        },
